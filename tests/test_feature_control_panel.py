...
async def main():
    active_insts = ["BTC-USDT-SWAP", "ETH-USDT-SWAP", "DOGE-USDT-SWAP"]
    ...
    for inst in active_insts:
        async with httpx.AsyncClient() as client:
            ...
            await show_status(client, f"Before {inst} Status")
            # 1) 初始 instruments
            # before = await _req(client, "GET", "/instruments", inst)
            ...
            if True:
                patch1 = {
                    "inst": inst,
                    "channels": {
                        "candles": {"fetch": True, "kind": "trade", "bars": ["15m", "30m", "1H", "4H"]},
                        "trades": {"fetch": True},
            ...
            await show_status(client, f"After {inst} instruments")
